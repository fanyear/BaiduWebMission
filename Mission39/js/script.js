window.onload = function() {
    ...
    window.onscroll = function(){
      fixElement("frame","fix")
    }
}

function fixElement(parentEle,id){
    var parentPos = document.getElementById(parentEle).offsetTop;
    var height = document.getElementById(parentEle).offsetHeight;
    var ele = document.getElementById(id);
    var scrollPos = document.body.scrollTop;
    var div = document.createElement("div")
    div.style.height = ele.offsetHeight + "px";
    if(scrollPos >=parentPos && scrollPos <= parentPos + height){
      if(ele.parentNode.getElementsByTagName("div").length == 0){
        ele.parentNode.insertBefore(div,ele)
      }
      ele.style.position = "fixed"
      ele.style.top = "0px"
    }else{
      if(ele.parentNode.getElementsByTagName("div")[0]){
        ele.parentNode.removeChild(ele.parentNode.getElementsByTagName("div")[0])
      }
